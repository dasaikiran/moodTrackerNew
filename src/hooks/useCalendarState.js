...
// EmojisList is included for structural reference if needed by handleDateClick,
// but the primary logic for selecting an emoji will be outside this hook.
...
const useCalendarState = () => {
  const [calenderList, setCalenderList] = useState(
    // Deep copy initialMonthsList to prevent mutation of the original
    // This is important if initialMonthsList is also exported and used elsewhere directly
    JSON.parse(JSON.stringify(initialMonthsList)),
  );
  const [month, setMonth] = useState(1); // Default month is January (1-indexed)

  const handleDateClick = useCallback(
    (dateId, currentMonthNumber, activeEmojiDetails) => {
      let oldEmojiName = null;
      let newEmojiName = null;

      setCalenderList(prevCalenderList => {
        const monthIndex = prevCalenderList.findIndex(
          m => m.month === currentMonthNumber,
        );
        if (monthIndex === -1) return prevCalenderList; // Month not found

        const updatedDates = prevCalenderList[monthIndex].dates.map(date => {
          if (date.id === dateId) {
            oldEmojiName = date.emojiName || null;
            if (date.emojiName === activeEmojiDetails.emojiName) {
              // Clear emoji if same emoji is clicked again
              newEmojiName = null;
              return { ...date, emojiUrl: '', emojiName: '' };
            }
            // Set new emoji
            newEmojiName = activeEmojiDetails.emojiName;
            return {
              ...date,
        ...
        const newCalenderList = [...prevCalenderList];
        newCalenderList[monthIndex] = {
          ...newCalenderList[monthIndex],
          dates: updatedDates,
        };
        return newCalenderList;
      });
      
      return { oldEmojiName, newEmojiName };
    },
    [], // No dependencies, setCalenderList updater form handles stale state
  );
  ...
  return {
    ...
    onLeftArrowClick,
    onRightArrowClick,
    initialMonthsList, // Exporting for other components like Reports
  };
};
